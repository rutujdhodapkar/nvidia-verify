...
// We read THAT tool's own schema and fill its required inputs generically.
async function resolveDynamicEnum(token, propName) {
  ...
  const eProps = enumTool.inputSchema?.properties || {};

  // Schema-driven fill: names hinting at which action/property they describe
  const args = {};
  for (const k of Object.keys(eProps)) {
    const kl = k.toLowerCase();
    if (!requiredString(enumTool, k) && !/search|query|filter/i.test(kl)) continue;
    if (/^tool$|tool_?name|action/.test(kl)) args[k] = currentToolName;
    else if (/prop|param|field/.test(kl)) args[k] = propName;
  ...
  const result = await rpc(token, {
    ...
    id: Date.now(),
    method: 'tools/call',
    params: { name: enumTool.name, arguments: args },
  });
  ...
  try {
    ...
    return Array.isArray(parsed) ? parsed : parsed?.data || parsed?.values || null;
  } catch {
    return rawText.trim() ? [{ value: rawText.trim(), label: rawText.trim() }] : null;
  }
}

let currentToolName = null;

function requiredString(tool, prop) {
  const req = tool.inputSchema?.required || [];
  const t = tool.inputSchema?.properties?.[prop]?.type;
  return req.includes(prop) && (t === 'string' || !t);
}
...
function buildArgs(tool, text, resolvedEnums) {
  ...
  // Seed required fields FIRST (some dynamic enums aren't listed in properties)
  for (const name of required) {
    ...
    } else if (ARG_TEXT_FIELDS.includes(name)) {
      args[name] = text;
    } else {
      args[name] = process.env[`ZAPIER_${name.toUpperCase()}`] || '';
    }
  }

  // Fill optional text-ish fields too
  for (const [name, spec] of Object.entries(props)) {
    if (name in args) continue;
    const type = spec?.type || (spec?.anyOf ? 'string' : spec?.enum ? 'string' : null);
    if (type === 'string' && ARG_TEXT_FIELDS.includes(name)) args[name] = text;
  }
  return args;
}
...
export async function postToLinkedinViaZapier({ token, text, pageId }) {
  ...
  // Resolve dynamic enums (e.g. company_id) — priority: pageId > enum lookup > known fallback
  const props = tool.inputSchema?.properties || {};
  const required = tool.inputSchema?.required || [];
  const resolvedEnums = {};
  ...
  if (!resolvedEnums.company_id && required.includes('company_id')) {
    const fallback = pageId || '134233993'; // devcraft-internships page (matches linkedin-poster.js)
    console.log(`      company_id unresolved via enums — using fallback: ${fallback}`);
    resolvedEnums.company_id = [{ value: fallback }];
  }

  const args = buildArgs(tool, text, resolvedEnums);
  console.log(`      Args: ${Object.entries(args).map(([k, v]) => `${k}=${k === 'comment' ? `<${String(v).length} chars>` : String(v).slice(0, 24)}`).join(', ')}`);
