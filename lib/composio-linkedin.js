...
export async function getAuthorUrn() {
  ...
  if (!urn) throw new Error(`No author identity in GET_MY_INFO: ${JSON.stringify(d).slice(0, 150)}`);
  return String(urn).startsWith('urn:') ? String(urn) : `urn:li:person:${urn}`;
}

// Verified member URN for this connection (Rutuj Dhodapkar)
const KNOWN_AUTHOR_URN = 'urn:li:person:WrAmP1oUE9';

// Posts a text update. Pass urn:li:organization:{id} to post on the company page.
export async function createLinkedInPost({ text, authorUrn }) {
  const author = authorUrn || process.env.LINKEDIN_AUTHOR_URN || KNOWN_AUTHOR_URN;
