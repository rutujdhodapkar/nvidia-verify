...
export async function sendEmail({ to, toName, subject, html, text, headers }) {
  ...
  // Brevo is the primary provider for both web and promo mails (as requested),
  // with Mailjet as a fallback when Brevo is exhausted or fails.
  const providers = ['brevo', 'mailjet'].filter(p =>
  ...
  for (const provider of providers) {
    try {
      ...
      if (provider === 'mailjet') {
        ...
        // Brevo expects plain object headers ({Name: value}), NOT the Mailjet array form.
        result = await sendBrevoEmail({ to, toName, subject, htmlContent: html, textContent: text, headers });
